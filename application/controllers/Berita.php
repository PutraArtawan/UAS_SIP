...
class Berita extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['hasil'] = $this->M_berita->get_all();
    ...
    public function tambah_berita()
    {
        ...
        $data['hasil'] = $this->M_berita->get_category();
        $this->load->view('admin/template/header', $data);
        $this->load->view('admin/template/sidebar', $data);
        $this->load->view('admin/template/topbar', $data);
        $this->load->view("admin/tambah_berita", $data,);
    ...
    {
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            $data_post = $this->M_berita->get_detail_by_id($id);
            if (count($data_post) > 0) {
                $aksi = $this->M_berita->update($id);
                if ($aksi) {
                    redirect('berita', 'refresh');
                }
            } else {
            ...
            $data['old_value'] = $this->M_berita->get_detail_by_id($id);
            $data['hasil'] = $this->M_berita->get_category();
            $data['user'] = $this->db->get_where('tb_user', ['email' =>
            $this->session->userdata('email')])->row_array();
